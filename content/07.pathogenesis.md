...
In models the quantities of basic reproduction (or reproductive) number and the timescale of infection have a large influence on the population level dynamics of epidemics.
...
R~0~ is a unitless number which in models is a combination of parameters principally related to the transmission rate (rate of infection transmitting interactions for an infectious individual) and the infectious period.
When R~0~ is greater than 1, there is potential for an epidemic to grow.
...
When R~t~ is greater than 1, an epidemic grows (i.e., the proportion of the population that's infectious increases); when R~t~ is smaller than 1, the proportion of the population that's infectious decreases.
...
Data and empirically determined biological mechanisms inform models, while models can be used to try to understand data and systems of interest or to try to make predictions on possible future dynamics, such as for capacity needs or for the comparison of predicted possible outcomes of different prevention and control strategies.
Beyond model predictions related to cases and deaths, infectious disease population models can also incorporate the dynamics of finer details such as possible genetic variations in pathogens or sometimes even possible evolutionary changes in pathogens due to relatively large numbers of pathogens which can evolve relatively quickly.
