...
Here, we describe efforts to adapt Manubot to handle the extreme case of the COVID-19 infodemic, with the objective of moving beyond simply reviewing preprints to develop a centralized platform for summarizing and synthesizing a massive amount of preprints, news stories, journal publications, and data.
Unlike prior collaborations built on Manubot, here,:w
 most contributors came from a traditional biological or medical background.
...
By recording the evolution of information over time and assembling a resource that auto-updated in response to the evolving crisis, Manubot offered particular value for managing rapid changes in scientific thought.
...
This update was achieved by implementing a query to clinicaltrials.gov that retrieved the XML metadata associated with each identifier using JavaScript [@url:https://github.com/zotero/translators/pull/2153]. 
...
All seven will be submitted to _mSystems_ as part of a special issue that provides support for evolving reviews so that they can continue to be updated as more information becomes available.
...
This feature and the other new features required for the COVID-19 project are now included in Manubot's rootstock.
...
Using CI, Manubot now checks that the manuscript was built correctly, runs spell-checking, and cross-references the manuscripts cited in this review.<!-- we probably can't include the appendix? original sentence ended with: as summarized in Appendix A and discussed in the project's issues and pull requests.-->
...
Through the development of cyberinfrastructure both to train novice users to interact with GitHub and to simplify workflows to allow them to receive many of the benefits of What You See Is What You Get platforms, we were able to adapt a powerful open publishing tool to harness the domain expertise of a large group of non-technical users and to respond to the flood of COVID-19 publications.
...
Additionally, because of the heavy emphasis on Word processing in biology, Manubot's ability to generate docx files was expanded to allow users to generate docx files containing only a section of the manuscript.
In our case, where the full project contained over 100,000 words, this allowed individual pieces to be shared widely.
...
Interested participants came from a wide range of backgrounds, and many of the responses emphasized a willingness to learn about new topics, especially COVID-19 and SARS-CoV-2 (Figure @fig:wordcloud).
...
Manubot provided a way for all contributors, including ECRs, to join a massive collaborative project but also demonstrate their individual contributions to the larger work and to gain experience with version control.
