...
Because the GitHub issues and comment systems are relatively similar to other common web activities, we found that authors learned these tools quickly.
...
Though only a fraction of potential contributors contributed to the text included in the manuscripts (Figure @fig:projectstats), many contributors remained engaged over the course of a full year (Figure @fig:projectdots).
...
The current project was managed through GitHub in the repository `greenelab/covid19-review` using Manubot [@doi:10.1371/journal.pcbi.1007128] to continuously generate a version of the manuscript online.
...
While Manubot manuscripts are written in markdown, they can be rendered in several formats that provide different advantages to users.
