...
The viral spike protein, _S_, plays a critical role in viral replication.
The N-terminal S1 domain used for binding and the C-terminal S2 domain for fusion [@doi:10.3390/v4061011].
...
Pathogenesis following exposure to a virus involves three major components: entry, replication, and spread [@isbn:0-9631172-1-1 {chap. 45}]. 
...
The S1 domain evolves more rapidly than the S2 domain, and different coronaviruses bind to difference receptors.
Betacoronaviruses are known to bind to CEACAM1, Neu 5,9 Ac2, and ACE2 [@doi:10.3390/v4061011].
During this third phase, the viral infection will begin to target specific tissues or organs, and can also pass form the host into the environment in a process known as viral shedding [@doi:10.3390/v4061011].
...
As described above, pathogenesis, or the entry of an antigen into host cells, involves three major components: entry, replication, and spread [@isbn:0-9631172-1-1 {chap. 45}].
...
The antibody-mediated response to an immune challenge occurs when a pathogen is detected in circulation by a phagocyte, or white blood cell.
Antigen-presenting phagocytes can engulf the pathogen, destroy it, and present the resulting peptides on the cell surface attached to MHC-II proteins.
B-lymphocytes, or B-cells, have antibodies bound to their membrane, with different B-cells having different membrane-bound antibodies.
A B-cell is able to bind to an antigen only if its antibody is a match.
In the case of a cornavirus, toll-like receptor 3 (TLR3) would be expected to recognize the double-stranded RNA (dsRNA) as a viral signature.
However, _in vitro_ analysis of nidoviruses including SARS-CoV suggests that these viruses induce double-membrane vesicles where viral replication can occur without the dsRNA signature being detected by the host immune system [@doi:10.1128/jvi.02501-05].
This protective envelope can therefore insulate these coronaviruses from detection by the innate immune response [@doi:10.1016/j.jpha.2020.03.001].
Coronaviruses such as SARS are also able to evade the humoral immune response through other mechanisms, such as inhibiting certain cytokine pathways or down-regulating antigen presentation by the cells [@doi:10.1128/jvi.02501-05].

This binding allows the B-cell to engulf the antigen through endocytocis, and once the antigen has been processed and presented on the cell surface, helper T-cells can bind to the MHC-II/antigen complex.
...
Antibodies, also known as immunoglobulins, belong to five classes that determine which antigens they can bind to.
Two of the most commonly discusses immunoglobulin classes are IgM, which are the first antibodies produced in response to an infection, and IgG, which are the most common antibodies found in the blood.
