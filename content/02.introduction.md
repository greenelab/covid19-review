...
We performed a correlation analysis of the host expression response (RNA fold change compared to mock infection) using the publicly available result files from Blanco-Melo et al. and Wyler et al. (Figure {@fig:cell-lines-moi}A) [@doi:10.1016/j.cell.2020.04.026; @doi:10.1101/2020.05.05.079194]. 
In general, we found high correlations among the same host (e.g. Calu-3 cells) irrespective of the study and virus infected, suggesting that the response is host specific.  
Moreover, COVID-19 patients have low correaltions with all the cells used (highest value of 0.29), suggesting that the models used may not necessarily imitate human response, therefore, appealing/asking/demanding for using additional models. 
