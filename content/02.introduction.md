...
The human immune response is double-pronged, with the innate and the adaptive immune response.
...
The antibody-mediated immune response is innate and provides a way through which the host can detect and respond to a pathogen in circulation.
...
Monocytes, which include both dendritic cells and macrophages, are both phagocytic white blood cells and professional antigen-presenting cells, which meaning that they can then present the resulting peptides on the cell surface using Major Histocompatability Complex II (MHC-II) proteins.
...
The binding of a pattern recognition receptor (PRR), such as a TLR, to the antigen allows the B-cell to engulf it via endocytocis.
...
Two of the most commonly discusses immunoglobulin classes are immunoglobulin M (IgM), which are the first antibodies produced in response to an infection, and immunoglobulin G (IgG), which are the most common antibodies found in the blood.
...
When an antigen succeeds in infiltrating the host's cells, T-cells are activated through the cell-mediated or adaptive immune response.
B-cells and helper T-cells recognize antigens presented by phagocytes on major histocompatability complex (MHC) II molecules.
However, all nucleated cells are able to present antigens via MHC I molecules to attract CD8+ T-cells and activate them as cytotoxic T-cells that destroy the cell.
...
While limited information is available about the host response to SARS-CoV-2, inferences can be made based on previous research into the immune response to SARS-CoV-1.
Research on SARS has revealed that presentation by MHC I molecules plays a larger role in the response to the virus than MHC II molecules [@doi:10.1016/j.jpha.2020.03.001].

To enter into a host cell, a virion must first bind to a receptor on the host cell surface.
For enveloped viruses such as coronaviruses, the viral membrane then fuses to the cellular membrame.
The viral spike protein controls this process, with the N-terminal S1 domain used for binding and the C-terminal S2 domain for fusion [@doi:10.3390/v4061011].
The S1 domain evolves more rapidly than the S2 domain, and different coronaviruses bind to difference receptors.
