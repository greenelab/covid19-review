...
While limited information is available about the host response to SARS-CoV-2, research in SARS-CoV suggests that the cell-mediated immune response is activated through antigen presentation by major histocompatability complex (MHC) I molecules, with MHC II also likely to play a role [@doi:10.1016/j.jpha.2020.03.001].
The antigen is then recognized by virus-specific T-cells, specifically cytotoxic T-lymphocytes [@doi:10.1016/j.jpha.2020.03.001].

b-cell works slightly differently:
- specific to certain epitopes (certain pathogens)
- variable portions are all different
- billions of combinations of proteins (variable ends of antibodies)
- part of surface (epitope) binds to variable sequence & starts activation process (binding sometimes sufficient to activation, definitely necessary)
- engulfed by b-cell, activates, proliferates
- some become plasma b-cells, some memory b-cells. plasma b-cells just produce tons of antibodies. can tag so that other macrophages can recognize them, eat them up, etc.
- b cell also does what the phagocytes do
- these are non-professional nad most cells are like this

-MHC II are recognized by helper T-cells
