...
It can even cause neurological complications [@doi:10.1016/j.bbi.2020.03.031], potentially including stroke, seizures or meningitis [@doi:10.1002/jmv.26000].
...
SARS-CoV-2 is a novel human coronavirus, following SARS-CoV-1 (SARS-CoV) and MERS-CoV to become the third coronavirus causing severe pneumonia in humans to emerge in the 21st century [@doi:10.1016/bs.aivir.2018.01.001]. 
...
Two studies profiled expression response after SARS-CoV-2 infection in multiple human cell lines (Figure {@fig:cell-lines-moi}A) [@doi:10.1016/j.cell.2020.04.026; @doi:10.1101/2020.05.05.079194]. 
The first study, which is by Blanco-Melo et al., performed an in-depth analysis of the transcriptional response to SARS-CoV-2 in comparison to other respiratory viruses, including MERS-CoV, SARS-CoV-1, human parainfluenza virus 3 (HPIV3), respiratory syncytial virus (RSV), and influenza A virus (IAV). 
...
As the viral receptor ACE2 has low expression in A549 cells, authors also supplemented A549 cells with a vector expressing mCherry or ACE2 (A549-ACE2). 
All infections were performed at a high multiplicity of infections (MOI, 2–5), except A549-ACE2 cells were infected with SARS-CoV-2 at both low (0.2 MOI) and high MOI (2 MOI).
Poly(A) bulk RNA sequencing (RNA-seq) of host cells was performed after 24 hours post-infection (hpi) of all the viruses except IAV (9-12 hours).
...
Additionally, transcriptional response to IAV in NHBE cells and nasal washes of ferrets was measured.
Differential expression (DE) analysis was carried out between infected and respective mock (control) cell condition.
DE results revealed that the transcriptional response of the cells with efficient SARS-CoV-2 replication is significantly different from the host response of all other viruses tested. 
A unique proinflammatory cytokine signature is present in both high and low multiplicity of infections (MOI). 
However, in contrast to high-MOI, low-MOI SARS-CoV-2-infected A549-ACE2 cells showed no significant IFN-I or IFN-III expression, suggesting that IFN induction is MOI dependent. 
...
Together, these results suggest that in contrast to common respiratory viruses, SARS-CoV-2 induces limited antiviral state with low IFN-I or IFN-III expression and a moderate IFN-stimulated genes (ISG) response. 
...
This hypothesis was further supported by a recent study [@doi:10.1101/2020.05.11.088179] that showed that the SARS-CoV-2 _ORF3b_ gene suppresses IFNB1 promoter activity (IFN-I induction) more efficiently than the SARS-CoV-1 _ORF3b_ gene, which is considerably longer.
...
They performed a comprehensive analysis of the transcriptional response to SARS-CoV-2 and SARS-CoV-1 at different hpi (4-36 hours) with an MOI of 0.3. 
Using poly(A) bulk RNA-seq, the authors found negligible susceptibility of H1299 cells (< 0.08 viral read percentage of total reads) in contrast to Caco-2 and Calu-3 cells (>10% of viral reads). 
Based on the visual inspection (microscopy images) and transcriptional profiling, authors showed distinct host responses after the infection. 
In contrast to Caco-2, SARS-CoV-2 infected Calu-3 cells showed signs of impaired growth and cell death at 24hpi, and a moderate IFN induction with a strong up-regulation of ISGs. 
Interestingly, the response is similar to SARS-CoV-2 infected Calu-3 cells at ~7-fold higher MOI performed by Blanco-Melo et al., suggesting that IFN induction is not MOI dependent in Calu-3 cells (in contrast to A549-ACE2 cells). 
...
We performed a correlation analysis of the host expression response (RNA fold change compared to the mock infection) using the publicly available result files from Blanco-Melo et al. and Wyler et al. (Figure {@fig:cell-lines-moi}B) [@doi:10.1016/j.cell.2020.04.026; @doi:10.1101/2020.05.05.079194]. 
In general, we found high correlations among the same host (e.g. Calu-3 cells) irrespective of the study and the virus infected, suggesting that the response is host-specific.  
Moreover, COVID-19 patients have low correaltions with all the cells used (highest value of 0.29), suggesting that the models used may not necessarily imitate the human response, therefore, appealing/asking/demanding for using additional models. 


![**Profiling and correlation of expression response after respiratory viruses infection in various hosts.** (A) Overview of hosts and respiratory viruses used in two studies [@doi:10.1016/j.cell.2020.04.026; @doi:10.1101/2020.05.05.079194]. MOI:Multiplicity of infections; PFU:Plaque-forming unit; a:ACE2-expressing A549 cells; b:Data used from GSE56192. (B) Pearson's correlation of expression response (Log2 fold change) between various hosts. Note that we used the data from bulk RNA-seq produced after the infection of unmodified viruses.](images/cell-lines-moi.jpg){#fig:cell-lines-moi}

[Start with scRNA-seq studies in healthy tissues (put this before Blanco-Melo et al), then summarize scRNA-seq studies in COVID-19 patients]
