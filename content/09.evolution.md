...
Throughout the pandemic, many hypotheses have been raised about factors that might make individuals more susceptible.
...
Today, in the age of next-generation sequencing, a common approach is instead to conduct genome-wide scans, where a large number of single-nucleotide variants (SNPs) are evaluated to identify regions of the genome associated with differences in a phenotype.
Genome-wide association scans (GWAS) in particular are a popular approach that employs this strategy.
...
Their analysis revealed an association between the SNP and severe COVID-19 outcomes. <!--the in-line stats are different than the table? are the in line
 stats for a model that didn't include age? because it's not significant in the table-->
A subsequent study claimed to support this result by comparing the frequency of the SNP in different groups to the COVID-19 case fatality rate in those groups; they examined SNPs in several candidate genes and identified an association with another SNP in _IFITM3_ [@doi:10.3390/genes12010042].
...
The narrative surrounding _IFITM3_ therefore reflects a broad methodological critique about candidate gene studies, where studies are often under-powere
d and results frequently fail to replicate.
