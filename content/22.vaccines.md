...
The major example of a VLP approach applied to COVID-19 comes from Covifenz, a VLP vaccine developed by Canadian company Medicago [@doi:10.1503/cmaj.1095992].
This vaccine was developed using plant-based VLP technology [@url:https://medicago.com/en/our-products/our-vaccines/covifenz-covid-19-vlp-vaccine] that the company had been investigating in order to develop a high-throughput quadrivalent VLP platform for influenza vaccines [@doi:10/gjn28x].
This approach utilizes _Nicotiana benthamiana_, an Australian relative of the tobacco plant, as an upstream bioreactor [@doi:10/gjn28x].
Specifically, genes from SARS-CoV-2 are inserted into a bacterial vector (_Agrobacterium tumefaciens_) that then infects the tobacco plant cells [@doi:10/gjn28x].
Expression of the genes causes the cells to produce proteins that present similar to the virion but do not contain any DNA, thereby mitigating any risk [@doi:10/gjn28x].

However, the worldwide footprint of this vaccine remains small, and as a result, only {{owid_VLP_count}} VLP vaccine has been approved with distribution in {{owid_VLP_countries}}.
