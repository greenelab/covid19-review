...
With these vaccines in place, one concern is how long they will remain effective, especially as the virus continues to evolve.
Since the start of this pandemic we have already seen multiple variants emerge: B.1.1.7 (emerged in the UK), B.1.351 (emerged in South Africa), P.1 (emerged in Brazil).

Viruses evolve or mutate at different rates.
Mutation rate is measured as the number of substitutions per nucleotide per cell infected (&mu_{s/n/c)) [@doi:10.1128/JVI.00694-10].
_c_ is estimated based on the viral titers at the start of infection and after infection, and viral yield.
_s/n_ is estimated based on the mutation frequency (number of mutations divided by the total number of nucleotides sequenced) and the total number of mutations detected by the sequencing method used.
Given these two calculations we can estimate mutation rate. 
RNA viruses tend to have mutation rates between 10^(-6) to 10^(-4) [@doi:10.1128/JVI.00694-10].
As a reference, influenza A virus has a mutation rate of 10^(-5) whereas the mutation rate of SARS-CoV-2 is lower, with (10^(-6)) mutation rate [@doi:10.24875/BMHIM.20000183].
This accumulation of mutations allows the virus to escape recognition by the immune system [@doi:10.1093/protein/gzw017].
These mutations are usually located in epitopes, which are regions of the virual antigen (i.e. spike protein) that are bound by antibodies.

The efficacy of vaccines depends on their ability to train the immune system to recognize the virus.
Therefore viruses can develop resistance to vaccines through the accumulation of mutations.
The lower mutation rate of SARS-CoV2 suggests the possibility of SARS-CoV2 vaccines having a more long-lasting effect compared to Influenza A vrius ones.

The vaccines in distribution have a similar efficacy against the B.1.1.7 variant, but reduced efficacy against the B.1.351 variant [@doi:10.1056/NEJMc2102017].
...
Previous research on computationally predicting efficacy of vaccine targeting Influenza A virus, might help complement efforts to monitor these types of viral outbreaks [@doi:10.1093/cid/ciy323].
To adapt, future vaccines may need to include multiple strains, booster shots may be required.
