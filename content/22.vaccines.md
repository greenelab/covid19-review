...
The clinical trial data surrounding these vaccines demonstrates that they significantly reduce the likelihood of contracting symptomatic COVID-19.
Therefore, the data clearly demonstrates an exceptional benefit to patients receiving the vaccine.
However, given that vaccination rates are unlikely to reach 100%, another concern is whether vaccinated individuals with or without symptoms of COVID-19 can still spread SARS-CoV-2.
This question must be broken down into several pieces.
...
However, the sample size in both of these studies was small, and a larger study found higher viral load in symptomatic than asymptomatic cases [@doi:10.3390/v13020304].
Therefore, the potential relationship between vaccination status, symptom presence, and viral load is not inherently obvious.
<!--Given that X% of vaccinated participants in the phase 2/3 clinical trials were diagnosed with COVID-19,-->
A related question is therefore whether vaccinated individuals with a SARS-CoV-2 infection (regardless of symptom status) are as contagious as unvaccinated individuals, which can be approximated by assessing whether SARS-CoV-2-positive vaccinated and unvaccinated individuals carry a comparable viral load approximated by cycle threshold (C~t~).
...
In order to assess the rate of asymptomatic infections in the vaccinated population, it is necessary to test asymptomatic individuals for the presence of SARS-CoV-2.
...
These findings therefore suggest that X% of SARS-CoV-2-positive vaccinated individuals are asymptomatic.

Another consideration is whether vaccinated individuals positive for SARS-CoV-2 carry a similar viral load to unvaccinated individuals.
...
C~t~ was analyze relative to when each patient received the first dose of the Pfizer mRNA vaccine.
...
A third study investigated viral load (as approximated by C~t~) for samples processed in a single laboratory during the summer of 2021 [@doi:10.1101/2021.07.31.21261387].
This study identified no significant differences in C~t~ among fully vaccinated and unvaccinated cases, but this study used samples sent for diagnosis and was not longitudinal.
