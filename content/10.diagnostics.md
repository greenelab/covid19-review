...
They have also been shown to work in one study using stool samples [@doi:10.1002/jmv.25742].
This has been applied to broader community surveillance rather than for patient diagnostics, in studies that tested wastewater. [@doi:10.1016/j.scitotenv.2020.138764]
...
#### Pooled PCR Testing Approaches

Due to limited supplies and the need for more tests, a number of labs have found ways to enable pooled or similar tests.
The first such result was a group at the Technion, which found they could pool up to 32 samples [@doi:10.1101/2020.03.26.20039438], followed by larger pooling in work utlizing slightly different methods at the University of Minnesota [@doi:10.1101/2020.04.02.022186].
...
False negatives are also a concern when sample collection (which can be difficult for nasopharyngeal cavity samples,) is done improperly.
...
Serological tests also require waiting until a patient has had an imune system reaction sufficient to detect, which means that these tests are of limited use for diagnosis in the early stages of an infection.
...
Clinical symptoms are similar enough to other types of pneumonia that purely symptomatic diagnosis is unlikely to be sufficient.
However, related approaches for diagnosis of cases exist, though most of these are only likely to identify serious and symptomatic cases.

X-ray diagnostics have high sensitivity but low specificity in some studies. [@doi:10.1148/radiol.2020200642]
Others have shown that some doctors seems to be able to better distinguish, though sensitivity in this case was lower [@doi:10.1148/radiol.2020200823].
...
Even using machine learning, this class of diagnostics are limited to confirming clinically severe cases, however, and will not detect asymptomatic or presymptomatic infections, which is critical for stopping spread.
