...
There are two main classes of diagnostic tests: molecular tests, which can diagnose an active infection by identifying the presence of SARS-CoV-2, and immunoassays, including serological tests that can assess whether an individual was infected in the past via the presence or absence of antibodies against SARS-CoV-2.
...
Tests for viral proteins typically use an antibody pair for detection in lateral flow tests (LFTs) [@doi:10.1007/s00216-010-3661-4].
...
Immunoassays, on the other hand, detect the presence of biological targets in a sample using approaches such as enzyme-linked immunosorbent assay (ELISA), lateral flow immunoassay (LFIA), and chemiluminescence immunoassay (CLIA) [@DOI:10.3389/fmolb.2021.682405; @doi:10.1093/ajcp/aqaa112].
These tests can provide rapid results.
Within this category, serological tests detect the presence of antibodies in blood plasma samples, providing insight into whether an individual has acquired immunity against SARS-CoV-2.
To distinguish past infection from vaccination, serological tests can be designed to detect antibodies that bind the nucleocapsid protein of the SARS-CoV-2 virus [@doi:10.1093/infdis/jiaa273].
...
In fact, while the specificity of the assay is extremely high (analytical specificity), the challenges of implementing testing introduce variability that results in a lower clinical specificity [@doi:10.1101/2020.04.26.20080911].
...
Through genomic surveillance, it has become possible to monitor the emergence of variants of interest (VOI) and variants of concern (VOC) that may pose additional threats due to increased contagiousness or virulence [@doi:10/hkwri; @url:https://cov-lineages.org].
...
Sequencing efforts are critically important to identifying variants containing mutations that might affect the reliability of molecular diagnostic tests [@doi:10.1038/s41576-021-00360-w; @doi:10.2807/1560-7917.ES.2020.25.39.2001650].
...
Countries have varied in their strategies to employ molecular testing as a tool to reduce viral transmission, even among high-income countries [@url:https://www.bloomberg.com/news/newsletters/2022-01-07/a-stark-contrast-between-the-u-s-and-europe-on-tests].
...
Although diagnostic tests based on the detection of genetic material can have high selectivity and sensitivity, they provide information only about active infection, and therefore offer just a snapshot-in-time perspective on the spread of a disease.
...
In this context, immunoassays, including serological tests, are more informative.
Immunoassays can be used to assess the presence of an antigen (e.g., the SARS-CoV-2 spike protein) or antibody in a sample.
Serological tests that assay antibodies in the serum are particularly useful for insight into population-level dynamics and can also offer a glimpse into the development of antibodies by individual patients during the course of a disease.

Immunoassays are designed to identify molecular indicators of SARS-CoV-2 infection such as evidence of an immune response to the SARS-CoV-2 virus.
One approach is to detect the antigen itself, but it is also possible to detect antibodies produced by the immune system in response to viral threat.
...
Since 2020, many immunoassays have been developed for COVID-19 [@url:https://www.centerforhealthsecurity.org/covid-19TestingToolkit/serology/Serology-based-tests-for-COVID-19.html] using several different technologies.

#### Lateral Flow Tests

Lateral flow tests (LFTs) provide distinct value relative to PCR tests.
They are used to detect current infection and can return results within 30 minutes.
They can also be performed without specialized equipment and at low cost.
...
Despite their reduced sensitivity relative to PCR tests, LFTs occupy an important niche in the management of SARS-CoV-2.
...
For COVID-19 diagnostics, ELISA can detect antibodies or all or part of the antigenic Spike protein [@doi:10.12688/wellcomeopenres.15927.1; @doi:10.1016/j.jim.2021.113182]. 

#### Chemiluminescence Immunoassay

Several approaches have been developed to detect SARS-CoV-2 antibodies.
An early study that was initially released on _medRxiv_ on February 25, 2020 presented a chemiluminescence immunoassay (CLIA) to identify COVID-19 cases.
This test uses a synthetic peptide derived from the amino acid sequence of the SARS-CoV-2 _S_ protein [@doi:10.1093/infdis/jiaa243].
This approach was highly specific to SARS-CoV-2 and detected IgM in 57.2% and IgG in 71.4% of sera samples from 276 COVID-19 cases confirmed with RT-qPCR.
IgG could be detected within two days of the onset of fever, but IgM could not be detected any earlier.
This pattern was consistent with observations in Middle East respiratory syndrome (MERS), which is also caused by an HCoV.
...
Differnces in limits of detection [@doi:10.2142/biophysico.bppb-v18.004].

Like molecular tests, immunoassays carry a number of limitations that influence their utility in different situations.
...
Due to the long incubation times and delayed immune responses of infected patients, immunoassays are insufficiently sensitive for a diagnosis in the early stages of an infection.
The limitations due to timing make these tests far less useful for enabling test-and-trace strategies.
<!--To Do: Add a sentence explaining the utility of serosurveys?-->

Given that immunity may wane over time, t
he duration of immunity might also vary with age [@doi:10.1038/s41590-021-01089-8] or ABO blood type [@doi:10.1080/21505594.2021.2019959].
...
<!--CT: 10.1101/2020.02.25.20027755 -->
