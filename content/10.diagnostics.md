...
Molecular tests are used to find specific sequences in a sample, which first requires getting a patient sample. 
Samples used for molecular tests include nasopharyngeal cavity samples, including throat wash and saliva [@doi:10/ggp4qx], or stool samples [@doi:10.1002/jmv.25742].
Molecular tests have false negative rates that depend on patient viral load, the testing target, and the sample type (nasopharyngeal swab, saliva, fecal, or otherwise.)
Molecular testing has also been applied to broader community surveillance by testing wastewater. [@doi:10.1016/j.scitotenv.2020.138764]
China has approved 11 different nucleic acid and reagent based tests [@url:http://www.nmpa.gov.cn/WS04/CL2056/375802.html].

#### Amplification and Identification Processes
Given a sample from a patient, tests require sample pre-processing, followed by library preparation, and then sequencing itself [doi:10.1016/j.biotechadv.2020.107537]. 
Pre-processing is needed to copy the sample in order to create DNA from the RNA sample, then to produce quantities sufficient for a test by amplifying the DNA.
...
RT-PCR tests are a type of test which uses reverse transcriptase to translate the viral RNA into DNA for pre-processing and uses PCR testing for sequencing.
There are different reagents used for library preparation which allow identifying one or more target section(s) with PCR.  [@doi:10.1021/acsnano.0c02624]
...
Around the same time as the above RT-PCR tests, researchers in China developed a quantitative real-time reverse transcription PCR (qRT-PCR) test to identify two regions of the viral genome, orf1b and N region [@doi:10.1093/clinchem/hvaa029].
...
These approaches are PCR-based but allow for more rapid scaling and higher efficiency for testing.
...
Due to the long incubation times and delayed immune responses of infected patients, serological tests are too insensitive for a diagnosis in the early stages of an infection.
...
Clinical symptoms are too similar to other types of pneumonia to be sufficient/reliable as a sole/single diagnostics criterion.
In addition, as noted above, identifying asymptomatic cases is critical.
Clinical symptom-based tests are only able to identify symptomatic cases, not presymptomatic or assymptomatic cases.
They are still important for clinical practice, and for reducing the need for other tests.

X-ray diagnostics have high sensitivity but low specificity in some studies [@doi:10.1148/radiol.2020200642].
Other studies have shown that specificity varies between radiologists, [@doi:10.1148/radiol.2020200823] though the sensitivity reported here was lower that in the previous paper.
...
This may still not recognize fully asymptomatic cases.
In addition, the widespread use of X-ray tests on otherwise healthy adults is likely inadvisable.
