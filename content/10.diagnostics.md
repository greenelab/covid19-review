...
Two of the primary metrics used to evaluate these tests are sensitivity and selectivity.
Sensitivity refers to a test's ability to correctly identify a true positive; for example, a test with 50% sensitivity might identify SARS-CoV-2 in only one of every two positive samples.
On the other hand, selectivity refers to how accurately a test is able to identify a negative sample as negative.
Here, this metric can be relevant both in terms of understanding the risk of false positives and in discussing whether a test is susceptible to identifying other coronaviruses.
