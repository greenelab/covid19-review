...
For infectious viral diseases, several strategies can be employed to identify compounds of interest [@doi:10.1016/j.tim.2018.04.004].
...
Computer-aided design then allowed for the development of a Michael acceptor inhibitor, now known as N3, to target Mpro, which is a protease critical to SARS-CoV-2 replication [@doi:10.1038/s41586-020-2223-y].
...
Molecular dynamic analysis has further demonstrate the potential for ebselen to bind to Mpro and disrupt the protease's enzymatic functions [@doi:10.1126/sciadv.abd0345]. 
...
High-throughput drug screens thus illustrate the issues inherent to drug discovery and repurposing efforts broadly.
...
In the case of phospholipidosis, the data-driven approach allowed for researchers to notice a shared property among many of the hits [@doi:10.1126/science.abi4708].
In this context, phospholipidosis is known to translate poorly from _in vitro_ to _in vivo_ research [@doi:10.1126/science.abi4708], so this insight is likely to be valuable in prioritizing candidate drugs.
In other cases, though, similar mechanisms can yield different results clinically; for example, remdesivir and favipiravir use similar mechanisms, but the former is considered a promising therapeutic while the latter is likely not (Table @tbl:summary-table). 
