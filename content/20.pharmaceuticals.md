...
Currently, TCZ is being administered either as a monotherapy or in combination with other treatments in {{ebm_tocilizumab_ct}} interventional COVID-19 clinical trials (Figure @fig:ebm-trials).
...
They were both developed by Regeneron in a parallel high-throughput screening (HTS) to identify neutralizing antibodies from either humanized mice or patient-derived convalescent plasma [@doi:10.1126/science.abd0827].
...
The drug development process is slow and costly and the development of compounds specifically targeted to an emerging viral threat is not a practical short-term solution.
Screening existing drug compounds for alternative indications is a popular alternative [@doi:10.1016/j.drudis.2019.06.014; @doi:10.1111/bph.13895; @doi:10.1039/D0CS01065K; @doi:10.1016/j.coi.2013.08.004].
HTS has been a goal of pharmaceutical development since at least the mid-1980s [@doi:10.1038/sj.bjp.0707373].
...
In many cases, these screens utilize large libraries that encompass a diverse set of agents varying in many pharmacologically relevant properties (e.g., [@url:https://www.chembridge.com/screening_libraries/diversity_libraries]).
The compounds inducing a desired effect could then be followed up on.
Around the turn of the millennium, advances in molecular biology allowed for HTS to shift towards emphasizing specific molecular targets that were hypothesized to have a desired effect and to screen for compounds interacting with the target.
These approaches both offer pros and cons, and today a popular view is that they are most effective in combination [@doi:10.1038/nrd4366; @doi:10.1016/j.chembiol.2015.11.008; @doi:10.1126/scitranslmed.aab1201].
...
Just as remdesivir was selected as a candidate antiviral because it is a nucleoside analog [@doi:10.1021/acs.jcim.0c00861], so too can high-throughput screens select libraries of compounds based on a molecular hypothesis.
Likewise, when the library of drugs is selected without basis in a potential mechanism, a screen can be considered hypothesis free [@doi:10.1021/acs.jcim.0c00861].
...
Such research can be driven by _in vitro_ or _in silico_ experimentation.
Computational analyses depend on identifying compounds that modulate pre-selected proteins in the virus or host.
As a result, they build on experimental research characterizing the molecular features of the virus, host, and candidate compounds [@doi:10.1016/j.coi.2013.08.004].
...
Both computational and experimental approaches facilitated the identification of compounds that might inhibit SARS-CoV-2 M^Pro^.
...
Today, like many other areas of biology, _in silico_ analyses have become increasingly popular and feasible through advances in biological big data [@doi:10.1021/acs.jcim.0c00861; @doi:10.1080/23808993.2019.1617632].
...
Resources such as the COVID-19 Drug and Gene Set Library, which at the time of its publication contained 1,620 drugs sourced from 173 experimental and computational drug sets and 18,676 human genes sourced from 444 gene sets [@doi:10.1016/j.patter.2020.100090], facilitate such discovery-driven approaches.
...
One example of a hypothesis-free screen for COVID-19 drugs comes from a PPI-network-based analysis that was published early in the pandemic [@doi:10.1038/s41586-020-2286-9].
...
Thus, this finding suggested that these screens were identifying compounds that shared a physiochemical property rather than a specific target [@doi:10.1126/science.abi4708].
...
It is possible that fluvoxamine also exerts an antiviral effect [@doi:10.3389/fphar.2021.652688].
...
Computational techniques were used to design compounds targeting M^Pro^ based on an understanding of how this protease aids viral replication, and M^Pro^ inhibitors remain promising candidates [@doi:10.1039/D0CS01065K], although the clinical trial data is not yet available.
Similarly, computational analysis correctly identified the Sigma-1 receptor as a protein of interest, and although the process of identifying which drugs might modulate the interaction led to an emphasis on candidates that ultimately have not been supported, fluvoxamine remains an appealing candidate.
...
Additionally, the literature used to identify and validate targets can be difficult to reproduce [@doi:10.1038/nrd3439-c1], which may pose challenges to target-based experimental screening and to _in silico_ screens. 
...
Concerns about the practicality of hypothesis-free screens to gain novel insights is underscored by the fact that very few or possibly no success stories have emerged from hypothesis-free screens over the past twenty years [@doi:10.1021/acs.jcim.0c00861].
...
As it stands, data-driven efforts to identify patterns in the results of computational screens allowed researchers to notice the shared property of cationic amphicility among many of the hits from computational screening analyses [@doi:10.1126/science.abi4708].
...
The unpredictable nature of success and failure in drug repurposing for COVID-19 thus highlights one of the tenets of phenotypic screening: there are a lot of "unknown unknowns", and a promising mechanism at the level of an MMOA will not necessarily propagate up to the pathway, cellular, or organismal level [@doi:10.1126/scitranslmed.aab1201].
...
While significant progress has been made in the first 18 months of the pandemic, the therapeutic landscape is likely to continue to evolve as more results become available from clinical trials and as efforts to develop novel therapeutics for COVID-19 progress.
