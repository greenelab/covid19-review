...
readURL <- function (query_date) {
  ...
  local_fname <- file.path("CDC_Data", paste("COVID19-pubs", 
                                             query_date, "xlsx", sep = "."))
  ...
  else if ( url.exists(remote_fname) ) { 
...
findpapers <- function (COVID19_data, keywords){
  ...
  write_tsv(hit_papers, 
            file.path("bytopic", 
                      paste(keywords[1], functional_date, "tsv", sep=".")))
  print(paste("There are", nrow(hit_papers), "papers related to", keywords[1], "as of", 
              Sys.time()))
}
